...
int main(void)
{
	...
	NRF_LOG_INFO("0: UWB LOG Initialization is DONE(err_code is 0x%x)",error_code);
	nrf_delay_ms(30);      
    NRF_LOG_FLUSH();
  ...
  /*Initialization UART*/
  boUART_Init ();
  NRF_LOG_INFO("Singled Sided Two Way Ranging Initiator Example \r\n");
  ...
  * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
  ...
  dwt_setrxtimeout(RESP_RX_TIMEOUT_UUS); // Maximum value timeout with DW1000 is 2.7ms  
